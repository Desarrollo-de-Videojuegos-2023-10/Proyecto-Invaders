...
def system_score_rendering(world: esper.World):
    ...
    for _, (c_score, c_s) in components:
        ...
        if not c_score.hiscore:
            c_s.surf = font.render("00" if c_score.score == 0 else str(c_score.score), True, pygame.Color(interface_cfg["normal_color"]["r"], interface_cfg["normal_color"]["g"], interface_cfg["normal_color"]["b"]))
